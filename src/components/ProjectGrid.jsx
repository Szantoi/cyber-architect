...
const ENRICHED_PROJECTS = [
  {
    ...
    techDeepDive: "A SpaceOS Nexus architektúra-elvek éles webes megvalósítása: React 18 + Node.js Express Clean Architecture rétegződés. Többszintű memória és indexelés (SQLite 3 WAL módban, FTS5 BM25 teljes szöveges és 128-dimenziós vektor hasonlósági keresés, Zero Raw Query szabállyal). A Google Drive import manuális admin művelet, alapértelmezetten pull-only: írásmentes dry-run előnézettel, lapozott fájlfeltárással, Drive file ID alapú forrásazonossággal, valamint ütközés- és hibariporttal. A platform emellett RFC 6238 TOTP 2FA adminisztrációs kaput, auditált SSE eseményfolyamot és Service Worker PWA offline tudásbázist használ.",
